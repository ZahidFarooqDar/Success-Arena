namespace SuccessArenaFoundation.Controllers.Token
{
    ...
    public class ExternalUserController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<TokenResponseSM>>> GoogleSignUp(string idToken, string companyCode)
        {
            ...
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "Id Token cannot be null or empty", "Id Token cannot be null or empty");
            }
            ...
            if (existingClientUser != null)
            {
                ...
                if (externalUser != null) // external user added / is already available
                    throw new SuccessArenaException(ApiErrorTypeSM.Success_NoLog, "Google Login Details Already Exist...Sign in Instead", "Google Login Details Already Exist. Please use login.");
                else // error in adding external user
                    throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "Internal error occured, try again after sometime. If problem persists, contact support.", $"Error in adding external user with id token {idToken}");
            }
            ...
            if (newUserSM == null)
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "Error in saving your details... Try Again", "Error in saving your details... Try Again");
            }
            else
            {
        ...
        public async Task<ActionResult<ApiResponse<TokenResponseSM>>> FacebookSignUp(string idToken, string companyCode)
        {
            ...
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log,
        ...
        private async Task<ActionResult<ApiResponse<TokenResponseSM>>> CreateTokenForUser(ClientUserSM clientUserSM, string companyCode)
        {
            ...
            var token = await _jwtHandler.ProtectAsync(_configuration.JwtTokenSigningKey, claims, new DateTimeOffset(DateTime.Now), new DateTimeOffset(expiryDate), "SuccessArenaKiller");
