...
namespace SuccessArenaBAL.AppUsers
{
    public partial class ApplicationUserProcess : LoginUserProcess<ApplicationUserSM>
    {
        ...
        public async Task<ApplicationUserSM> AddApplicationUser(ApplicationUserSM applicationUserSM, string roleType)
        {
            ...
            {
                throw new SuccessArenaException(ApiErrorTypeSM.InvalidInputData_NoLog, @"Please Provide details to Add new User", "Please Provide details to Add new User");
            }
            ...
            {
                throw new SuccessArenaException(ApiErrorTypeSM.InvalidInputData_NoLog, "User With EmailId already existed...Try another EmailId", "User With EmailId already existed...Try another EmailId");
            }
            if (existingUserWithLoginId != null)
            {
                throw new SuccessArenaException(ApiErrorTypeSM.InvalidInputData_NoLog, "User With LoginId already existed...Try another LoginId", "User With LoginId already existed...Try another LoginId");
            }
        ...
        public async Task<ApplicationUserSM> UpdateApplicationUser(int userId, ApplicationUserSM objSM)
        {
            ...
            if (objDM == null)
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"User Not Found", "User Not Found");
            }
            if (!objSM.LoginId.IsNullOrEmpty())
            {
                if (objSM.LoginId != objDM.LoginId && objSM.LoginId.Length < 5)
                {
                    throw new SuccessArenaException(ApiErrorTypeSM.InvalidInputData_NoLog, @"Please provide LoginId with minimum 5 characters", "Please provide LoginId with minimum 5 characters");
                }
            }
            ...
            if (objDM != null)
            {
                ...
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"Something went wrong while Updating Application User Details", "Something went wrong while Updating Application User Details");
            }
            else
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"Application User not found: ", "Data to update not found, add as new instead.");
            }
        }
        ...
        private async Task<bool> UpdateProfilePicture(int userId, string base64String)
        {
            ...
            throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "Cannot Update Profile Picture", "Cannot Update Profile Picture");
        }
