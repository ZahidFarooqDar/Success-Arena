namespace SuccessArenaBAL.AppUsers
{
    /*public partial class ClientUserProcess : LoginUserProcess<ClientUserSM>
    {
        ...
        public async Task<IEnumerable<ClientUserSM?>> GetUsersByCompanyId(int companyId, int skip, int top)
        {
            ...
            if (companyId <= 0)
            {
                throw new SuccessArenaException(ApiErrorTypeSM.InvalidInputData_NoLog, @"Please provide valid Client Company Id", "Please provide valid Client Company Id");
            }
        ...
        public async Task<(LoginUserSM, int, string)> GetCompanyAutomationUserById(int id)
        {
            ...
            if (user == null)
                throw new SuccessArenaException(ApiErrorTypeSM.NoRecord_Log, $"User not found in db with id {id}", $"User not found in db with id {id}");
        ...
        public async Task<BoolResponseRoot?> AddNewUser(ClientUserSM signUpSM, string companyCode, string link)
        {
            if (signUpSM == null)
            {
                throw new SuccessArenaException(ApiErrorTypeSM.InvalidInputData_NoLog, "Please provide details for Sign Up", "Please provide details for Sign Up");
            }
            if (signUpSM.LoginId.Length < 5)
            {
                throw new SuccessArenaException(ApiErrorTypeSM.InvalidInputData_Log, "Please provide LoginId with minimum 5 characters", "Please provide LoginId with minimum 5 characters");
            }

            if (signUpSM.EmailId.IsNullOrEmpty())
            {
                throw new SuccessArenaException(ApiErrorTypeSM.InvalidInputData_NoLog, "Please provide EmailId ", "Please provide EmailId");
            }
            if (signUpSM.FirstName.IsNullOrEmpty())
            {
                throw new SuccessArenaException(ApiErrorTypeSM.InvalidInputData_NoLog, "Please provide FirstName ", "Please provide FirstName");
            }
            ...
            if (existingUserWithEmail != null)
            {
                if (existingUserWithEmail.ClientCompanyDetailId == existingCompanyDetail.Id)
                {
                    ...
                    {
                        throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "The email address you entered is already in use. Please log in or use a different email address.");
                    }

                    if (existingUserWithEmail.LoginId == signUpSM.LoginId)
                    {
                        throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "The login ID you entered is already taken. Please log in or choose a different login ID.");
                    }
                }
            ...
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "The provided Login ID is already in use. Please use a different Login ID.");
            }
            ...
            using (var transaction = await _apiDbContext.Database.BeginTransactionAsync())
            {
                ...
                {
                    throw new SuccessArenaException(ApiErrorTypeSM.Access_Denied_Log, "Password Is Mandatory", "Password Is Mandatory");
                }
                ...
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "Something went wrong while adding your details, Please try again later");
            }
        }
        ...
        public async Task<BoolResponseRoot> ResendEmailVerification(EmailConfirmationSM objSM, string link)
        {
            ...
            {
                throw new SuccessArenaException(ApiErrorTypeSM.InvalidInputData_NoLog, "Please provide your email", "Please provide your email");
            }

            if (link.IsNullOrEmpty())
            {
                throw new SuccessArenaException(ApiErrorTypeSM.InvalidInputData_NoLog, "Something went wrong while generating link to send email verification", "Something went wrong while generating link to send email verification");
            }
        ...
        public async Task<ClientUserSM> UpdateClientUser(int userId, ClientUserSM objSM,bool isSocialMediaUpdation = false)
        {
            if (userId == null)
            {
                throw new SuccessArenaException(ApiErrorTypeSM.NoRecord_NoLog, $"Please Provide Value to Id", $"Please Provide Value to Id");
            }

            if (objSM == null)
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"Nothing to Update", "Nothing to Update");
            }
            ...
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Access_Denied_Log,
            ...
            if (objDM != null)
            {
                ...
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"Something went wrong while Updating Client Details", "Something went wrong while Updating Client Details");
            }
            else
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"Client not found: ", "Data to update not found, add as new instead.");
            }
        }
        ...
        public async Task<DeleteResponseRoot> DeleteClientUserById(int id)
        {
            ...
            if (client != null)
            {
                ...
                *//*var existingClientsWithCompanyCount = await _apiDbContext.ClientUsers.AsNoTracking()
                ...
                {
                    throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"User cannot be deleted as we should have atleast one user associated with company", "User cannot be deleted as we should have atleast one user associated with company");
                }
        ...
        public async Task<BoolResponseRoot> SendLoginIdToEmail(EmailConfirmationSM objSM)
        {
            if (string.IsNullOrWhiteSpace(objSM.Email))
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "Please provide an email ID. Email cannot be empty.");


            var user = await GetClientUserByEmail(objSM.Email);
            if (user == null)
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "This email is not associated with any registered account. Please sign up to create your account.");
            }
            ...
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "Unable to send the email. Please try again later.");
            }
        }
        ...
        public async Task<BoolResponseRoot> SendResetPasswordLink(ForgotPasswordSM forgotPassword, string link)
        {
            ...
            if (string.IsNullOrWhiteSpace(forgotPassword.UserName))
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "User Name cannot be empty.");
            ...
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"No ClientUser with username '{forgotPassword.UserName}' found.");
            }
        }
        ...
        public async Task<BoolResponseRoot> UpdatePassword(ResetPasswordRequestSM resetPasswordRequest)
        {
            ...
            if (forgotPassword.Expiry < DateTime.Now)
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"Link expired.", $"Password reset link expired.");
            }
            if (!string.IsNullOrWhiteSpace(resetPasswordRequest.NewPassword))
            {
                if (string.IsNullOrEmpty(forgotPassword.UserName))
                {
                    throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"No ClientUser with username '{forgotPassword.UserName}' found.", $"No ClientUser with username '{forgotPassword.UserName}' found.");
                }
                ...
                if (user != null)
                {
                    ...
                    {
                        throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"Please don't use old password, use new one.", $"Please don't use old password, use new one.");
                    }
                    else
                    {
            ...
            else
            {
                ...
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "Password can not be empty", "Password should not contain Spaces.");
            }
        }
        ...
        {
            if (string.IsNullOrWhiteSpace(objSM.Email))
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "Email cannot be empty.");
            ...
            if (user == null)
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"No User with email '{objSM.Email}' found.", $"No User with email '{objSM.Email}' found.");
            }
        ...
        private (string email, string pwd) ValidateUserFromDatabaseandGetEmail(ForgotPasswordSM forgotPassword)
        {
            ...
            if (string.IsNullOrWhiteSpace(user.ClientUser.EmailId))
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"Please Update Email For ClientUser With Username '{forgotPassword.UserName}'.");
        ...
        public async Task<BoolResponseRoot> SendEmailVerificationLink(EmailConfirmationSM objSM, string link)
        {
            ...
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"No User with email '{objSM.Email}' found.");
            }
        }
        /// <summary>
            ...
            *//*if (forgotPassword.Expiry < DateTime.Now)
            ...
            if (!string.IsNullOrWhiteSpace(sm.Email))
            {
                ...
                if (user != null)
                {
                    ...
                    throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"Something went wrong while verifying your email, Please try again later");
                }
                else
                {
                    throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, $"No ClientUser with email '{sm.Email}' found.", $"No ClientUser with username '{sm.Email}' found.");
                }
            }
            else
            {

                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "Email can not be empty", "Email can not be empty");
            }
        }
        ...
        private async Task<bool> UpdateProfilePicture(int userId, string base64String)
        {
            ...
            if (objDM == null)
            {
                throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "Client not found...Please check Again", "Client not found...Please check Again");
            }
            if (!objDM.ProfilePicturePath.IsNullOrEmpty())
            {
                imageFullPath = Path.GetFullPath(objDM.ProfilePicturePath);
            ...
            throw new SuccessArenaException(ApiErrorTypeSM.Fatal_Log, "", "Cannot Update Profile Picture");
        }
